...
require_once("../classes/frame_master.class.php");

$sbg_obj = new frame_master();
...
switch ($submit_action)
{
	case "preview":
		...
		if($edit_id <= 0)
		{
			header("location:frame_master.php");
			exit();
		}
	...
	case "save":
		...
		{//need to add the record
			...
			if($sbg_obj->insert())
				$redirect_url = "frame_master.php";
			else
				$redirect_url = "frame_master.php?submit_action=add";
		...
		{//update the record
			...
			else
			{
				...
				$redirect_url = "frame_master.php?submit_action=edit&" . $sbg_obj->primary_fld . "=" . $_REQUEST[$sbg_obj->primary_fld];
				}
		} 
	...
	case "delete":
		...
		if($id <= 0)
		{
			header("location:frame_master.php?page_id=$page_id");
			exit();
		}
		else
		{
...
switch ($display_what)
{
	...
	case "detail_frm";
		require_once("../forms/admin/frame_master_detail_frm.php"); 
		break;
	
	case "list_frm";
		require_once("../forms/admin/frame_master_list_frm.php"); 
		break;

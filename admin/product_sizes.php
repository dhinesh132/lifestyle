...
require_once("../classes/product_sizes.class.php");
require_once("../components/fckeditor/fckeditor.php"); 
$prod_obj = new product_sizes();
...
switch ($submit_action)
{
	case "preview":
		...
		if($edit_id <= 0)
		{
			header("location:product_sizes.php");
			exit();
		}
	...
	//$GLOBALS['site_config']['debug']=1;
		...
		{//need to add the record
			...
			if($prod_obj->insert())
				$redirect_url = "product_sizes.php";
			else
				$redirect_url = "product_sizes.php?submit_action=add";
		...
		{//update the record
				...
				else
				{
					...
					$redirect_url = "product_sizes.php?submit_action=edit&" . $prod_obj->primary_fld . "=" . $_REQUEST[$prod_obj->primary_fld];
				}
...
switch ($display_what)
{
	...
	case "detail_frm";
		require_once("../forms/admin/product_sizes_detail_frm.php"); 
		break;
	
	case "list_frm";
		require_once("../forms/admin/product_sizes_list_frm.php"); 
		break;

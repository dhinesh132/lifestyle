...
switch ($submit_action)
{
	...
	case "save":
		...
		{//need to add the record
			...
			if($prod_obj->insert())
				$redirect_url = "publications.php";
			else
				$redirect_url = "publications.php?submit_action=add";
		...
		{//update the record
 			if(isset($_REQUEST['sub_action']) && $_REQUEST['sub_action'] =='Quantity'){
				...
				else
				{
					...
					$redirect_url = "publications.php?submit_action=edit&" . $prod_obj->primary_fld . "=" . $_REQUEST[$prod_obj->primary_fld];
				}
			}
		} 
	...
	case "delete":
		...
		if($id <= 0)
		{
			header("location:publications.php?Id=$Id");
			exit();
		}
		else
		{
...
switch ($display_what)
{
	...
	case "detail_frm";
		require_once("../forms/admin/publications_detail_frm.php"); 
		break;
	
	case "list_frm";
		require_once("../forms/admin/publications_list_frm.php"); 
		break;

<div class="producfilter">
          ...
          <div class="w-tab-content">
           <?php $group_res = $db_con_obj->fetch_flds("groups", "*", "GroupStatus =1 order by DisplayOrder DESC"); 
            ...
            <div class="w--tab-active w-tab-pane" data-w-tab="<?php echo $group_data->UniqueKey;?>">
              ...
              <?php 
			  $prod_res = $db_con_obj->fetch_flds($prod_obj->cls_tbl,"Id, EnName,Price,Quantity,Types,Material,Function,ProdStatus,Image,UniqueKey,EnShortDesc","concat(',',Groups,',') Like '%,".trim($group_data->GroupId).",%'  order by DisplayOrder desc");
	...
	$fun_str=''; while($fun_prod_data = mysql_fetch_object($fun_res[0])){
		$fun_str .= $fun_prod_data->EnName.", ";
	 }  echo substr($fun_str,0,-2)?><br><br><?php echo trim_text($prod_data->EnShortDesc,30)?></p>
                    </div>
                    <div class="popbtns"><!--<a class="popbtn-item w-inline-block" href="<?php echo $GLOBALS['site_config']['site_path'];?>product/<?php echo $prod_data->UniqueKey?>"><img src="images/cart-icon.png" width="22"></a>--><a class="popbtn-item w-inline-block" href="<?php echo $GLOBALS['site_config']['site_path'];?>product/<?php echo $prod_data->UniqueKey?>"><img src="images/info.png" width="23"></a></div>
                  </div>
                  <div class="prod-img-con"><img class="prom-img" src="<?php echo $disp_img;?>" alt="<?php echo display_field_value($prod_data,"Name");?>">
                   <?php if($dis_res[1] >0){ ?>

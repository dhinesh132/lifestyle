...
if($edit == 1 && $edit_id > 0)
{
	...
	$res = $type_obj->fetch_record($edit_id);
	$type_obj = set_values($type_obj, "db", $res[0]);

}


if(isset($_SESSION['ses_temp_type_obj']) && is_array($_SESSION['ses_temp_type_obj']))
{
	$type_obj = set_values($type_obj,"ses",$_SESSION['ses_temp_type_obj']);
}

?>
<div class="whitebox mtop15">
<form action="" method="post" enctype="multipart/form-data" name="<?php echo $type_obj->frm_name; ?>" onSubmit="return check_form(window.document.<?php echo $type_obj->frm_name; ?>);">
<table width="65%" border="0" cellspacing="0" cellpadding="2" align="center" class="tableborder_new">
          ...
          <tr valign="top"> 
            ...
            <td width="77%"><input type="text" name="EnName" value="<?php echo stripslashes($type_obj->EnName['value']); ?>" class="mediumtxtbox">   </td>
    	 </tr>
        <tr valign="top"> 
            <td  height="30" class="postaddcontent"><span class="whitefont">Alias URL </span></td>
            <td width="60%"><input type="text" name="UniqueKey_temp" value="<?php echo stripslashes($type_obj->UniqueKey['value']); ?>" class="mediumtxtbox">   </td>
    </tr>
            ...
            <td><select name="TypeStatus" class="mediumtxtbox">
			<option value="0" <?php echo ($type_obj->TypeStatus['value'] == 0)?"selected":""; ?>>In-Active</option>
                <option value="1" <?php echo ($type_obj->TypeStatus['value'] == 1)?"selected":""; ?>>Active</option>
    ...
    <tr valign="top" class="postaddcontent"> 
          <tr valign="top" class="postaddcontent"> 
            <td colspan="2"><div align="center"> 
               ...
               <input type="hidden" name="CreatedDate" value="<?php echo (strlen($type_obj->CreatedDate['value']) > 0)?$type_obj->CreatedDate['value']:date("Y-m-d H:i:s"); ?>">
                <input type="hidden" name="TypeId" value="<?php echo $type_obj->TypeId['value']; ?>">
              </div></td>
          </tr>
          <tr> 
...
<script language="javascript">
frm_obj = window.document.<?php echo $type_obj->frm_name; ?>;	
</script>